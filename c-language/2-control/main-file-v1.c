...
void calculate_1_x(int i, double* y, double* x) {
	if (i == 0) {
		*x = 1.0;
	}
	else {
...
void calculate_1_y(int i, double *y, double *x) {
	if (i == 0) {
		*y = 0.14;
	}
	else {
		*(y+i) = *(y + i - 1) + sin(*(x + i - 1)) - 0.5 * *(x + i);
	}
};
...
void task_1(void) {
	...
	double *x = malloc(15 * sizeof(double));
	double *y = malloc(15 * sizeof(double));
