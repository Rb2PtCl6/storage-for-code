...
void test(int* arrayT, int size, int ideal) {
	if (arrayT == NULL || size < 1) {
		printf("\n\n%s\n\n", "Что-то не так с переданными данными!");
	} else {
		int recived = fold(arrayT, size, mul);
		printf("%s %d", "a = [",arrayT[0]);
		for (int i = 1; i < size; i++) {
			printf(", %d", arrayT[i]);
		}
		printf("%s\n\n", " ]");
		printf("%s %d; %s %d", "Желаемый результат:", ideal, "Полученный результат:", recived);
		if (ideal == recived) {
			printf("\n\n%s\n\n", "Тест пройден!");
		}
		else {
...
void task_1(void) {
	...
	for (int i = 0; i < mx2; i++) {
		for (int j = 0; j < mx2; j++) {
			...
			if (is_random) {
				tmp_dbl = insert_handy_input_2("a", i, j).value;
			}
...
void task_3(void) {
	...
	struct test_struct data[5] = { {NULL, 0, 0} };

	data[0].size = 3;
	data[0].ideal = 9;

	data[1].arr = malloc(2 * sizeof(int));
	data[1].arr[0] = 3;
	data[1].arr[1] = 4;
	data[1].size = -2;
	data[1].ideal = 12;

	data[2].arr = malloc(3 * sizeof(int));
	data[2].arr[0] = 1;
	data[2].arr[1] = 2;
	data[2].arr[2] = 3;
	data[2].size = 3;
	...
	data[3].arr = malloc(2 * sizeof(int));
	data[3].arr[0] = 3;
	data[3].arr[1] = 4;
	data[3].size = 2;
	data[3].ideal = 12;

	data[4].arr = malloc(3 * sizeof(int));
	data[4].arr[0] = 9;
	data[4].arr[1] = 0;
	data[4].arr[2] = 10;
	data[4].size = 3;
	...
	for (int i = 0; i < 5; i++) {
		printf("%s %d\n", "Тест", i+1);
		test(data[i].arr, data[i].size, data[i].ideal);
	}
	if (debug) {
